#!/usr/bin/env python3
"""
Qwen3-VL GGUF Export
====================
Converts the fine-tuned Qwen3-VL-2B (HuggingFace format) → GGUF for llama.cpp.
...
Usage:
    python 07c_export_qwen3vl_gguf.py
    python 07c_export_qwen3vl_gguf.py --from-mlx ../models/merged/qwen3vl-vision-merged
    python 07c_export_qwen3vl_gguf.py --quant-types Q4_K_M Q8_0
    python 07c_export_qwen3vl_gguf.py --deploy
"""
...
def find_llama_cpp():
    ...
    for p in candidates:
        if (p / "convert_hf_to_gguf.py").exists():
            return p
    # Return first candidate as default (setup_llama_cpp will clone here)
    return candidates[0]
...
MERGED_MODEL_DIRS = [
    Path("../models/merged/qwen3vl-vision-merged"),
    Path("../models/lora_adapters/qwen3vl-vision-merged"),
]
...
def setup_llama_cpp():
    ...
    if not QUANTIZE_BIN.resolve().exists():
        ...
        if detect_cuda():
            ...
            # Use the correct CUDA toolkit if multiple are installed
...
def export_projector(model_dir: Path, output_path: Path):
    ...
    Qwen3-VL uses a ViT-based vision encoder with RoPE-2D.
    The projector maps vision embeddings to the LM's embedding space.
    """
    ...
    if alt_mmproj_script.exists():
        ...
        print("  → Use the base mmproj GGUF or convert manually")
        return False
...
def deploy_to_app(gguf_path: Path, mmproj_path: Path, app_models_dir: Path):
    ...
    # Qwen3-VL is pro tier → server downloadable
...
def main():
    parser = argparse.ArgumentParser(
        description="Export fine-tuned Qwen3-VL to GGUF for llama.cpp")
    ...
    if model_dir is None:
        # Auto-detect merged model from known locations
    ...
    if model_dir is None or not model_dir.exists():
        ...
        print("    1. python 04b_train_qwen3vl_vision.py")
        print("    2. python 06_merge_lora.py --model qwen3vl")
        sys.exit(1)

    output_dir = args.output_dir
    output_dir.mkdir(parents=True, exist_ok=True)

    base_name = "Qwen3-VL-2B-finetuned"
    ...
    if not args.skip_projector:
        ...
        if not export_projector(model_dir, mmproj_path):
            # Fallback: look for existing Qwen3-VL mmproj
            candidates = glob.glob(str(Path("../../models/mmproj-*qwen*").resolve()))
            if not candidates:
                candidates = glob.glob(str(Path("../../ocula_app/assets/models/mmproj-*qwen*")))
            if candidates:
    ...
    # Summary
    print(f"\n{'═' * 60}")
    print(f"  QWEN3-VL GGUF EXPORT COMPLETE")
    print(f"{'═' * 60}")
    print(f"  Output: {output_dir}")
    for f in sorted(output_dir.glob(f"{base_name}*.gguf")):
        size_mb = f.stat().st_size / (1024 * 1024)
        print(f"    • {f.name} ({size_mb:.1f} MB)")
    for f in sorted(output_dir.glob("mmproj-Qwen3*.gguf")):
        size_mb = f.stat().st_size / (1024 * 1024)
        print(f"    • {f.name} ({size_mb:.1f} MB)")
    print(f"\n  To deploy: python 07c_export_qwen3vl_gguf.py --deploy")
    print(f"{'═' * 60}")
